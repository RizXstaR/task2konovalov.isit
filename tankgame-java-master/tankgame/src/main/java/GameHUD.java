    ...
    public void redraw(BufferedImage world) {
        ...
        for (int i = 0; i < playerGraphics.length; i++) {
            if (!this.players[i].isLoser()) {
                ...
                playerGraphics[i].drawString("Игрок " + (i + 1), 32, 40);
                playerGraphics[i].drawString("Оружие:", 32, 96);
                ...
                playerGraphics[i].setColor(Color.WHITE);
                playerGraphics[i].drawString("Нажми ESC чтобы выйти из игры", 12, this.playerInfo[i].getHeight() - 28);
                playerGraphics[i].drawString("Нажми F5 для перезапуска игры", 12, this.playerInfo[i].getHeight() - 12);

                Font bigFont = new Font("Courier New", Font.BOLD,72);
                playerGraphics[i].setFont(bigFont);
                playerGraphics[i].setColor(Color.RED);
                playerGraphics[i].drawString("ГЕЙМ", this.playerInfo[i].getWidth() / 2 - 100, this.playerInfo[i].getHeight() / 2 - 20);
                playerGraphics[i].drawString("ОВЕР", this.playerInfo[i].getWidth() / 2 - 100, this.playerInfo[i].getHeight() / 2 + 50);
            }
        }
